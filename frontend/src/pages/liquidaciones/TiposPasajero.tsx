...
import type { TipoPasajero } from '@/types'
import { useTiposPasajero } from '@/hooks/modules'
...
export default function TiposPasajero() {
  const tipos = useTiposPasajero()
  const { data, isLoading, error } = tipos.useList()
  const createMutation = tipos.useCreate()
  const updateMutation = tipos.useUpdate()
  const deleteMutation = tipos.useRemove()
  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<TipoPasajero | null>(null)
  ...
  const handleEdit = (item: TipoPasajero) => { setEditing(item); reset({ codigo: item.codigo, nombre: item.nombre, activo: item.activo }); setOpen(true) }
  ...
  const columns: Column<TipoPasajero>[] = [
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Tipos de Pasajero" subtitle="Clasificación para liquidación de tasas" action={<Button onClick={handleAdd}><Plus className="h-4 w-4" /> Nuevo Tipo</Button>} />
      {isLoading && (
      ...
      <CrudModal open={open} onOpenChange={setOpen} title={editing ? `Editar: ${editing.codigo}` : 'Nuevo Tipo de Pasajero'} isEditing={!!editing} onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Código</label>
            <Input {...register('codigo')} placeholder="Ej: ADULTO" className="font-mono uppercase" />
            {errors.codigo && <p className="text-xs text-rose-500">{errors.codigo.message}</p>}
          </div>
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Nombre</label>
            <Input {...register('nombre')} placeholder="Ej: Adulto" />
      ...
      <ConfirmDialog open={deleteId !== null} onOpenChange={(v) => { if (!v) setDeleteId(null) }} onConfirm={handleDelete} title="Eliminar Tipo" message="¿Está seguro de eliminar este tipo de pasajero?" />
    </div>
  )
}
