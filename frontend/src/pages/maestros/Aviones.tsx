...
import { useAeronaves } from '@/hooks/modules'
...
type FormValues = z.infer<typeof avionSchema>

export default function Aviones() {
  const aeronaves = useAeronaves()
  const { data = [], isLoading, error } = aeronaves.useList()
  const createMutation = aeronaves.useCreate()
  const updateMutation = aeronaves.useUpdate()
  const deleteMutation = aeronaves.useRemove()
  ...
  const filtered = data.filter(
    (item) =>
      ...
      (item.tipoCodigo || '').toLowerCase().includes(search.toLowerCase()),
  )
  ...
  const handleEdit = (item: Aeronave) => {
    setEditing(item)
    reset({ matricula: item.matricula, modelo: item.tipoCodigo || '', fabricante: '', capacidad_pasajeros: 0 })
    setOpen(true)
  }
              ...
              <TableBody>
                {filtered.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.tipoCodigo || '-'}</TableCell>
                    <TableCell>-</TableCell>
                    <TableCell>-</TableCell>
                    <TableCell>
                      <span
