...
import { useFabricantes } from '@/hooks/modules'
...
export default function Fabricantes() {
  ...
  const { data = [], isLoading, error } = fabricantes.useList()
  const createMutation = fabricantes.useCreate()
  const updateMutation = fabricantes.useUpdate()
  const deleteMutation = fabricantes.useRemove()

  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<Fabricante | null>(null)
  ...
  const handleEdit = (item: Fabricante) => {
    setEditing(item)
  ...
  const columns: Column<Fabricante>[] = [
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Fabricantes" subtitle="Fabricantes de aeronaves" action={
      ...
      {!isLoading && !error && (
        <DataTable columns={columns} data={data} keyExtractor={(i) => i.id} searchKeys={['codigo', 'nombre']} searchPlaceholder="Buscar fabricante..." />
      )}

      <CrudModal open={open} onOpenChange={setOpen} title={editing ? `Editar: ${editing.codigo}` : 'Nuevo Fabricante'} isEditing={!!editing} onSubmit={handleSubmit(onSubmit)}>
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Código</label>
            <Input {...register('codigo')} placeholder="Ej: BOE" className="font-mono uppercase" />
            {errors.codigo && <p className="text-xs text-rose-500">{errors.codigo.message}</p>}
          </div>
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-700">Nombre</label>
            <Input {...register('nombre')} placeholder="Ej: Boeing" />
          ...
          <div className="col-span-2 flex items-center p-3 bg-slate-50 rounded-lg">
            <Controller name="activo" control={control} render={({ field }) => <ToggleSwitch field={field} label="Fabricante activo" />} />
          </div>
        </div>
      </CrudModal>
      <ConfirmDialog open={deleteId !== null} onOpenChange={(v) => { if (!v) setDeleteId(null) }} onConfirm={handleDelete} title="Eliminar Fabricante" message="¿Está seguro de eliminar este fabricante?" />
    </div>
  )
}
