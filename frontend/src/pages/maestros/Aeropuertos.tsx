...
export default function Aeropuertos() {
  ...
  const { data = [], isLoading, error } = aeropuertos.useList()
  const createMutation = aeropuertos.useCreate()
  const updateMutation = aeropuertos.useUpdate()
  const deleteMutation = aeropuertos.useRemove()

  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<Aeropuerto | null>(null)
  ...
  const onSubmit = async (form: AeropuertoFormValues) => {
    if (editing) {
  ...
  const handleEdit = (item: Aeropuerto) => {
    setEditing(item)
    reset({
      codigoOACI: item.codigoOACI,
      codigoIATA: item.codigoIATA,
      nombre: item.nombre,
      ciudad: item.ciudadNombre || '',
      pais: item.paisNombre || '',
      activo: item.activo,
      controlAdministrativo: item.controlAdministrativo,
      opera24Horas: item.opera24Horas,
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={handleAdd} disabled={createMutation.isPending || updateMutation.isPending} className="bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-200 transition-all">
                ...
                <TableBody>
                  {filtered.map((item) => (
                    <TableRow key={item.id} className="group transition-colors hover:bg-slate-50/80">
                      ...
                      <TableCell className="text-slate-600">{item.ciudadNombre || item.paisNombre ? `${item.ciudadNombre || ''}, ${item.paisNombre || ''}` : '-'}</TableCell>
                      <TableCell>
                        {item.opera24Horas ? (
